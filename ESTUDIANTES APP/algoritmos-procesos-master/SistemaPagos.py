...
class SistemaPagos:
    def __init__(self, gestor_estudiantes):
        ...
        self.mensualidad_base = 675.00  # Monto base de la colegiatura
    ...
    def calcular_proximo_vencimiento(self):
        hoy = datetime.now()
        if hoy.day > 10:  # Suponiendo vencimiento día 10 de cada mes
            próximo_mes = hoy.replace(day=1) + timedelta(days=32)
            próximo_mes = próximo_mes.replace(day=10)
        else:
            próximo_mes = hoy.replace(day=10)
